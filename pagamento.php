<?php
    ...
    if(isset($_POST["criar"]) && !empty($_POST["criar"])){
        unset($_POST["criar"]);
        $criar = new Criar("pagamento", $_POST);
        $x = $criar->criar();
        echo $criar;
    }
    ...
    if(isset($_POST["editar"]) && !empty($_POST["editar"])){
        unset($_POST["editar"]);
        $editar = new Editar("pagamento", $_POST, $_POST["pagamento_id"]);
        $tab = $editar->editar();
        echo $editar;
    }

    if(isset($_POST["excluir"]) && !empty($_POST["excluir"])){
        $apagar = new Excluir("pagamento", $_POST["excluir"]);
        ...
        <table>
            <caption>
                ...
                <form method="POST" action="formCriar.php">
                    ...
                    <input name="criar" type="hidden" value="pagamento">
                </form>
            </caption>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        <td><?= $valor["pagamento_id"]?></td>
                        <td><?= $valor["cliente_id"]?></td>
                        <td><?= $valor["funcionario_id"]?></td>
                        <td><?= $valor["aluguel_id"]?></td>
                        <td><?= $valor["valor"]?></td>
                        <td><?= $valor["data_de_pagamento"]?></td>
                        <td><?= $valor["ultima_atualizacao"]?></td>
                        <td>
                            <form method="POST" action="formEditar.php">
                                <button type="submit">Editar</button>
                                <input name="editar_id" type="hidden" value="<?= $valor["pagamento_id"]?>">
                                <input name="editar_nome" type="hidden" value="<?= "pagamento"?>">
                            </form>
                            <form method="POST" action="pagamento.php">
                                <button type="submit">Excluir</button>
                                <input name="excluir" type="hidden" value="<?= $valor["pagamento_id"]?>">
                            </form>
                        </td>
                    </tr>
                    <?php } ?>
