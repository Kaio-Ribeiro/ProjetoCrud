<?php
    ...
    if(isset($_POST["criar"]) && !empty($_POST["criar"])){
        unset($_POST["criar"]);
        $criar = new Criar("categoria", $_POST);
        $x = $criar->criar();
        echo $criar;
    }
    ...
    if(isset($_POST["editar"]) && !empty($_POST["editar"])){
        unset($_POST["editar"]);
        $editar = new Editar("categoria", $_POST, $_POST["categoria_id"]);
        $tab = $editar->editar();
        echo $editar;
    }

    if(isset($_POST["excluir"]) && !empty($_POST["excluir"])){
        $apagar = new Excluir("categoria", $_POST["excluir"]);
        ...
        <table>
            <caption>
                ...
                <form method="POST" action="formCriar.php">
                    ...
                    <input name="criar" type="hidden" value="categoria">
                </form>
            </caption>
            <thead>
                <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="formEditar.php">
                                ...
                                <input name="editar_id" type="hidden" value="<?= $valor["categoria_id"]?>">
                                <input name="editar_nome" type="hidden" value="<?= "categoria"?>">
                            </form>
                            <form method="POST" action="categoria.php">
                                <button type="submit">Excluir</button>
                                <input name="excluir" type="hidden" value="<?= $valor["categoria_id"]?>">
                            </form>
                        </td>
                    </tr>
                    <?php } ?>
